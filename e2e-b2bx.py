...
def setHostEntry(user,password,machine):
        print "Setting host entries on "+machine
        ...
        sshHost(user,password,machine," cat /etc/hosts|grep -q 'cloudprimary.us.oracle.com';stat=$?;if [ $stat  != 0 ];then echo '"+data1+"' >> /etc/hosts;else sed -i.bak '/cloudprimary.us.oracle.com/d' /etc/hosts;echo '"+data1+"' >> /etc/hosts;fi")
        sshHost(user,password,machine," cat /etc/hosts|grep -q 'oid.us.oracle.com';stat=$?;if [ $stat  != 0 ];then echo '"+data2+"' >> /etc/hosts;else sed -i.bak '/oid.us.oracle.com/d' /etc/hosts;echo '"+data2+"' >> /etc/hosts;fi")
        sshHost(user,password,machine," cat /etc/hosts|grep -q 'idm.us.oracle.com';stat=$?;if [ $stat  != 0 ];then echo '"+data3+"' >> /etc/hosts;else sed -i.bak '/idm.us.oracle.com/d' /etc/hosts;echo '"+data3+"' >> /etc/hosts;fi")
        sshHost(user,password,machine," cat /etc/hosts|grep -q 'ohs.us.oracle.com';stat=$?;if [ $stat  != 0 ];then echo '"+data4+"' >> /etc/hosts;else sed -i.bak '/ohs.us.oracle.com/d' /etc/hosts;echo '"+data4+"' >> /etc/hosts;fi")
        sshHost(user,password,machine," cat /etc/hosts|grep -q 'db.us.oracle.com';stat=$?;if [ $stat  != 0 ];then echo '"+data4+"' >> /etc/hosts;else sed -i.bak '/db.us.oracle.com/d' /etc/hosts;echo '"+data5+"' >> /etc/hosts;fi")


def PrepareReport(Label):
        with open(homeDir+"/tmp/Report.txt",'w') as f:
                url1=os.environ.get('IDMpod01')+"-"+os.environ.get('IDMtenant01')+".data."+options.mach2
                url2=os.environ.get('IDMpod02')+"-"+os.environ.get('IDMtenant02')+".data."+options.mach2
                url3=os.environ.get('IDMpod03')+"-"+os.environ.get('IDMtenant03')+".data."+options.mach2
                f.write("****************** DAAS MULTINODE SETUP ENVIRONMENT DETAILS*******************\n\n\n")
                f.write("\n\nDAAS LABEL: "+Label+"\n")
                f.write("\n\n#Host File Entries:\n")
                f.write(socket.gethostbyname(options.mach2)+"   "+url1+"\n")
                f.write(socket.gethostbyname(options.mach2)+"   "+url2+"\n")
                f.write(socket.gethostbyname(options.mach2)+"   "+url3+"\n")
                f.write("\n\n#DB Details")
                f.write("\nDB Host: "+options.mach2)
                f.write("\nSID/port : orcl/1521")
                ...
                f.write("User Name: avuser\n")
                f.write("Password : welcome1\n")
                f.write("\n##OFFLINE ENVIRONEMNT ACCESS DETAILS\n")
                f.write("DaaS Webclient URL: http://"+socket.gethostname(options.mach2)+":7005/data/ui\n")
                f.write("User: daas_admin\n")
                f.write("Password: Welcome1\n")
                f.write("\nIndexing Pipeline URL : http://"+socket.gethostname(options.mach2)+":10003/insights/internal/task\n")
                f.write("Indexing Pipeline Configure URL : http://"+socket.gethostname(options.mach2)+":10003/ws_utc/?wsdlUrl=http://"+socket.gethostname(options.mach2)+":10003/insights/internal/config?WSDL\n")
                f.write("\n##DAAS WLS Admin details\n")
                f.write("WLS Admin console URL : http://"+socket.gethostname(options.mach2)+":7001/console\n")
                f.write("username: weblogic\n")
                f.write("password: welcome1\n")
                f.write("\n##Indexing pipeline WLS Admin details\n")
                f.write("WLS Admin console URL : http://"+socket.gethostname(options.mach2)+":10001/console\n")
                f.write("username: weblogic\n")
                f.write("password: welcome1\n")
...
def setup():
        try:
                ...
                if stat:
                        ...
                        setHostEntry(options.mach2user,options.mach2pass,options.mach2)
                        stat=startThread()
                        if stat:
                        ...
                        if 'setup' in options.action:
                                ...
                                if status1:
                                        ...
                                        if status2:
                                                ...
                                                #Send Report
                                                PrepareReport(os.environ.get('Label'))
