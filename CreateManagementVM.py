...
parser.add_option('--bfr',help="BigFileRepo URL",action="store",dest="bfrurl")

(options, args) = parser.parse_args()

if options.servicename is None or options.customeruser is None or options.podname is None or options.type is None or  options.chefserver is None or options.passwd is None or options.chefpasswd is None or options.chefuser is None or options.nimbulaapi is None or options.bfrurl is None:
...
DirSecIpIntra = "0.0.0.0/0"
dirManagementVmServerDataSize = '60'
...
if os.path.isdir(AUTO_WORK+'/app/managementvm') == 'True':
	os.rename(AUTO_WORK+'/app/managementvm',AUTO_WORK+'/app/managementvm'+'_OLD')
...
hostname =options.type + '-'+options.chefuser+'-'+options.podname.lower()+'-'+'mvm1'+'.'+options.servicename+'.oracleinternal'+options.type+'.oraclecorp.com'
...
ReplaceUserInput(ManagementJSON,'PLACEHOLDER4',options.podname.lower())
#ReplaceUserInput(ManagementJSON,'PLACEHOLDER5',dirManagementVmServerDataSize)
ReplaceUserInput(ManagementJSON,'PLACEHOLDER5',DataBag)
ReplaceUserInput(ManagementJSON,'PLACEHOLDER6',DirSecIpIntra)
...
Value4='  "dirRepoUrl" : "'+options.bfrurl+'",'
Value5='  "dirSecIpPublic" : "0.0.0.0/0",'
InsertValue(22,Value1)
InsertValue(25,Value2)
InsertValue(29,Value3)
InsertValue(19,Value4)
InsertValue(37,Value5)


#Call doSCP function to copy files
try:
	if options.type in 'ucf2c' and options.chefserver in 'grill.us.oracle.com':
...
#if Chef User is opcmats, use paasusr to copy the opserver artifacts
	elif options.chefuser in 'opcmats':
		print "\nCopying /etc/ogucf2c from "+options.chefserver+" Server"
		doScp('paasusr',options.chefpasswd,options.chefserver,'/etc/og',AUTO_WORK+'/app/managementvm/opsserver_artifacts/og')
        	print "\nCopying /usr/local/devops/bin from "+options.chefserver+" Server"
        	doScp('paasusr',options.chefpasswd,options.chefserver,'/usr/local/devops/bin',AUTO_WORK+'/app/managementvm/opsserver_artifacts')
        	print "\nCopying management_data_bag_item.json from "+options.chefserver+" Server"
        	doScp('paasusr',options.chefpasswd,options.chefserver,'/home/mats/management_data_bag_item.json',AUTO_WORK+'/app/managementvm/opsserver_artifacts')
	else:
		print "Unable to copy opserver artifacts from "+options.chefserver
		sys.exit(-1)
except:
	print "Unable to copy opserver artifacts from "+options.chefserver
	sys.exit(-1)
...
print "\nNIMBULA_API has been set to: "+os.getenv('NIMBULA_API')
print "\nNIMBULA_USER  has been set to: "+os.getenv('NIMBULA_USER')

#Running service-security.py script
#if os.path.isfile(AUTO_WORK+'/app/managementvm/output/service-security.py') is True:
...
if os.path.isfile(AUTO_WORK+'/app/managementvm/output/pod-bringup.py') is True:
